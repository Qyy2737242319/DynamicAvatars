...
import torch
import numpy as np
from flame_utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
import os
from flame_utils.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
# from pytorch3d.transforms import quaternion_multiply
from roma import quat_product, quat_xyzw_to_wxyz, quat_wxyz_to_xyzw
from flame_utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from flame_utils.graphics_utils import BasicPointCloud
from flame_utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def prune_points(self, mask):
        if self.binding is not None:
            ...
            counter_prune.scatter_add_(0, binding_to_prune.to(torch.int64), torch.ones_like(binding_to_prune, dtype=torch.int32, device="cuda"))
        ...
        if self.binding is not None:
            # Toyota Motor Europe NV/SA and its affiliated companies retain all intellectual property and proprietary rights in and to the following code lines and related documentation. Any commercial use, reproduction, disclosure or distribution of these code lines and related documentation without an express license agreement from Toyota Motor Europe NV/SA is strictly prohibited.
            self.binding_counter.scatter_add_(0, self.binding[mask].to(torch.int64), -torch.ones_like(self.binding[mask], dtype=torch.int32, device="cuda"))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        if self.binding is not None:
            ...
            self.binding_counter.scatter_add_(0, new_binding.to(torch.int64), torch.ones_like(new_binding, dtype=torch.int32, device="cuda"))#int32
