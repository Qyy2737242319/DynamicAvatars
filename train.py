...
from flame_scene.discriminator import Discriminator_512,Discriminator_new
...
class WebUI:
    def __init__(self, cfg, dataset) -> None:
        ...
        for i in tqdm.tqdm(range(len(self.json_data['camera_indices']))):  # frames
        ...
        with self.server.add_gui_folder("Render Setting"):
            ...
            self.finetune_mode = self.server.add_gui_slider(
                "snapshot finetune", min=-1, max=16, step=1, initial_value=0
            )
        ...
        with self.server.add_gui_folder("Edit Setting"):
            ...
            self.gan_lambda = self.server.add_gui_slider(
                "gan lambda", min=0.00, max=2.00, step=0.01, initial_value=0.45
            )
            ...
            with self.server.add_gui_folder("Advanced Options"):
                ...
                self.edit_train_steps = self.server.add_gui_slider(
                    "Total Step", min=0, max=2000000, step=100, initial_value=1000 #1500
                )
                ...
                with self.server.add_gui_folder("Learning Rate Scaler"):
                    ...
                    self.gs_lr_end_scaler = self.server.add_gui_slider(
                        "XYZ LR End", min=0.0, max=5.0, step=0.1, initial_value=1
                    )
                    self.color_lr_scaler = self.server.add_gui_slider(
                        "Color LR", min=0.0, max=5.0, step=0.1, initial_value=1
                    )
                ...
                with self.server.add_gui_folder("Loss Options"):
                    ...
                    self.lambda_anchor_color = self.server.add_gui_slider(
                        "Lambda Anchor Color", min=0, max=500, step=1, initial_value=100
                    )
                    self.lambda_anchor_geo = self.server.add_gui_slider(
                        "Lambda Anchor Geo", min=0, max=500, step=1, initial_value=10
                    )
                    self.lambda_anchor_scale = self.server.add_gui_slider(
                        "Lambda Anchor Scale", min=0, max=500, step=1, initial_value=10
                    )
    ...
    def edit(self, edit_cameras, train_frames, train_frustums, origin_frames=None):
        ...
        if self.gan_loss.value == 1:
            self.D_net = Discriminator_new(6, 8).cuda()
        ...
        for step in tqdm.tqdm(range(self.edit_train_steps.value)):
            ...
            if self.gan_loss.value == 1:
                ...
                d_loss = (D_criterion(d_true, true_label) + D_criterion(d_false, false_label)) * self.gan_lambda.value
            ...
            if step % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    if not os.path.exists("./perspect"):
                        os.makedirs("./perspect")

                    rendering = self.render(edit_cameras[7], train=True)["comp_rgb"]
                    loss, rendering, gt_image, d_image = self.guidance(rendering, 7, step)

                    cv2.imwrite(f"./perspect/render_{step}.jpg",
                                cv2.cvtColor(255 * rendering.detach().cpu().numpy().squeeze(), cv2.COLOR_BGR2RGB))
                    cv2.imwrite(f"./perspect/gt_{step}.jpg",
                                cv2.cvtColor(255 * gt_image.detach().cpu().numpy().squeeze(), cv2.COLOR_BGR2RGB))
